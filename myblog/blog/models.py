...
class MyUser(models.Model):
	...
	qq = models.CharField(max_length=20, blank=True, null=True, unique=True,
		verbose_name='QQ号码')
...
class ArticleManager(models.Manager):
    def distinct_date(self):
        ...
        for date in date_list:
        	# date1 = date['date_publish'].strftime('%Y/%m文章归档')
        	date1 = date['date_publish'].strftime('%Y-%m')
        	date2 = date1 + '文章归档'
	        if date2 not in distinct_date_list:
	            distinct_date_list.append(date2)
	        # dadada = date['date_publish']
	        # distinct_date_list.append(date2)
